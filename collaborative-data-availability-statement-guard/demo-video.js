...
<head>
  <meta charset="utf-8">
  <title>Collaborative data availability statement guard demo</title>
  <style>
...
<body>
  ...
  <script>
    ...
    const checks = [
      ["Availability sections", "Data and code statements must be complete before export."],
      ["Repository evidence", "Accession IDs, licenses, and de-identification evidence are validated."],
      ["Reviewer access", "Restricted artifact links must stay valid through the review window."],
      ["Collaborator approvals", "Data steward and code owner approvals gate the final manuscript package."]
    ];
    ...
    function draw(frame) {
      ...
      ctx.fillText("Data Availability Statement Guard", 48, 64);
      ctx.font = "19px Arial";
      ctx.fillStyle = "#475569";
      ctx.fillText("SCIBASE #12 collaborative editor export-readiness demo", 48, 98);
      ...
      ctx.font = "15px Arial";
      ctx.fillText("Synthetic data only. No journal systems, repositories, identity services, storage APIs, or live manuscript export calls.", 48, 504);
    }
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "data-availability-demo-"));
